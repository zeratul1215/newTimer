...
const Timer = () => {
  ...
  useEffect(() => {
    if(seconds == 0){//当时间归零的时候，判断是否是由于倒计时归零，
      setIsTimeUp(true);
      if(isRunning){
        setShouldReset(true);
        setIsRunning(false);
  ...
  // 时间格式化 用来显示非编辑状态下的时间
  const formatTime = useCallback((s: number) => {//将秒数转换为hh:mm:ss格式
    s = Math.ceil(s / 10);
  ...
  // 解析 hhmmss 字符串为秒
  const parseTime = useCallback((str: string) => {//将用户输入的时间转化为秒数
  ...
  const handleInputBlur = useCallback(() => {
    ...
    if (parsed !== null && parsed >= 0 && parsed <= 99 * 3600 * 10 + 59 * 60 * 10 + 59 * 10) {
      setSeconds(parsed);
  ...
  const renderInputValue = useCallback((val: string) =>
    val.replace(/(\d{2})(\d{2})(\d{2})/, '$1:$2:$3'), []);

  return (
    <div className={cls(
      styles.wholeContainer,
      isRunning ? styles.wholeContainerRunning : shouldReset ? styles.wholeContainerShouldReset : styles.wholeContainerIdle
    )}>
      ...
      <div className={styles.bottomBar}>
        {!shouldReset && <Button className={styles.startBtn} onClick={isRunning ? handlePause : handleStart}>
          {isRunning ? '⏸' : '▶️'}
        </Button>}
